...
PLOTS = [
    ...
    #dict(key='o_random/agent_col_rate', name='Agent collision rate (random)', label='Average rate'),
    #dict(key='o_random/agent_success_rate', name='Agent success rate (random)', label='Average rate'),
    #dict(key='o_static_same_goal/agent_col_rate', name='Agent collision rate (same goal)', label='Average rate'),
    #dict(key='o_static_same_goal/agent_success_rate', name='Agent success rate (same goal)', label='Average rate'),

    dict(key='reward/reward', name='Reward', label='Average reward'),
]
...
def extract(experiments):
    KEY_REMAP = {
        'static_diff_goal/distance_to_goal_1s': 'o_random/distance_to_goal_1s',
        'static_same_goal/distance_to_goal_1s': 'o_static_same_goal/distance_to_goal_1s',

        'static_diff_goal/agent_col_rate': 'o_random/agent_col_rate',
        'static_diff_goal/agent_success_rate': 'o_random/agent_success_rate',
        'static_same_goal/agent_col_rate': 'o_static_same_goal/agent_col_rate',
        'static_same_goal/agent_success_rate': 'o_static_same_goal/agent_success_rate'
    }
    ...
    for tmp_id in range(len(PLOTS)):
        search_key = PLOTS[tmp_id]['key']
        if search_key not in keys:
    # 如果找不到 key，看是否有重命名版本
            for alt in KEY_REMAP:
                if KEY_REMAP[alt] == search_key and alt in keys:
                    search_key = alt
                    break
        key_idx = keys.index(search_key)
...
def main():
    ...
    legend_name = [
    	'1. Full', 
    	'2. W/O replay buffer', 
    	'3. W/O multi-head attention', 
    	'4. W/O SDF-based obstacle obs (base policy)'
    ]
    all_experiment_dirs = {}
